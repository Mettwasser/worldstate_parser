...
use crate::{
    ...
    target_types::worldstate_types::{
        alert::{Alert, AlertMissionInfo, AlertMissionReward},
...
impl Resolve<ContextRef<'_>> for MissionRewardUnmapped {
    ...
    fn resolve(self, ctx: ContextRef) -> Self::Output {
        AlertMissionReward {
            ...
            counted_items: self.counted_items.resolve(ctx),
        }
    }
}
...
pub struct MissionRewardUnmapped {
    ...
    #[serde(default)]
    pub items: Vec<InternalPath<resolve_with::LanguageItems>>,

    #[serde(default)]
    pub counted_items: Vec<CountedItemUnmapped>,
}
